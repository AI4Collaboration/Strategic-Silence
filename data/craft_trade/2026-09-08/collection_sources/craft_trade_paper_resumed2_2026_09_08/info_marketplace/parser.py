...
# Capture a complete, consistently quoted message, preserving contractions and
# opposite quote characters inside its body. One capture keeps existing indices.
_QUOTED_MESSAGE = r'''("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')'''
...
def _parse_report(text: str, sender: str, round_num: int, is_public: bool, recipient: str | None) -> Report | None:
    ...
    report_match = re.search(
        rf'REPORTS?\s+(?:about\s+)?([^:]+?)\s*:\s*{_QUOTED_MESSAGE}',
        text,
        re.IGNORECASE
    )
...
def _parse_promise(text: str, sender: str, round_num: int, is_public: bool, recipient: str | None) -> Promise | None:
    ...
    promise_match = re.search(
        rf'PROMISES?\s+(?:to\s+)?(Agent_\d+)\s*:\s*{_QUOTED_MESSAGE}.*?by\s+round\s+(\d+)',
        text,
        re.IGNORECASE
    )
    ...
    if promise_match:
        ...
        commitment = promise_match.group(2)[1:-1].strip()
    ...
    promise_match = re.search(
        rf'(?:to\s+)?(Agent_\d+)\s*:\s*PROMISES?\s+{_QUOTED_MESSAGE}.*?by\s+round\s+(\d+)',
        text,
        re.IGNORECASE
    )
    ...
    promise_match = re.search(
        rf'PROMISES?\s+(?:to\s+)?(Agent_\d+)\s*:\s*{_QUOTED_MESSAGE}',
        text,
        re.IGNORECASE
    )
    ...
    promise_match = re.search(
        rf'(?:to\s+)?(Agent_\d+)\s*:\s*PROMISES?\s+{_QUOTED_MESSAGE}',
        text,
        re.IGNORECASE
    )
    ...
    # Format 5: common compact variants from smaller instruction models:
    #   [PROMISE] Agent_0: "I will gather food"
    #   [PROMISE] Agent_0, I will gather food
    #   [PROMISE] Agent_0 will gather food
    promise_match = re.search(
        r'PROMISES?\]?\s+(?:to\s+)?(Agent_\d+)(?:\s*[:,]\s*|\s+)(.+)$',
        text,
        re.IGNORECASE,
    )
    if promise_match:
        target = promise_match.group(1)
        commitment = promise_match.group(2).strip().strip('"\'')
        deadline = re.search(r'\bby\s+round\s+(\d+)\b', commitment, re.IGNORECASE)
        by_round = int(deadline.group(1)) if deadline else round_num + 3
        if deadline:
            commitment = commitment[: deadline.start()].rstrip(" ,.;")
        if commitment:
            return Promise(
                sender=sender,
                target=target,
