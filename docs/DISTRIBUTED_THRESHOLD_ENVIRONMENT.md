...
Raw traces from this initial, pre-fix diagnostic are under
`results/distributed_threshold/pilot_v2_*`. They establish environment viability
and the prospective-rule effect, but the raw-versus-attributed contrast should
be replaced by a post-fix run with yoked exposure.

## Post-fix yoked-exposure pilot

The identical DeepSeek V4 Flash design was rerun after adding the guaranteed
practice exposure. Raw traces are under
`results/distributed_threshold/pilot_v3_yoked_*`.
