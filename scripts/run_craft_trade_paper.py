...
def prepare(out):
 ...
 config=dict(protocol='craft_trade_paper_v2',model='openai/gpt-5.4',reasoning_effort='low',rounds=10,agents=4,
  cap_usd=BUDGET_USD,prior_preflight_reserved_usd=0,total_authorized_usd=BUDGET_USD,max_output_tokens=3000,goal_directives=GOALS,conditions=CONDITIONS,
  original_source='Original user-supplied ICLR strategic-silence paper; see published snapshot configuration for hash',
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('mode',choices=['prepare','run']);p.add_argument('--budget-usd',type=float);p.add_argument('--output',type=Path,default=ROOT/'results/craft_trade_paper_v2_2026_09_08');args=p.parse_args()
 if args.mode=='run' and (args.budget_usd is None or not 0 < args.budget_usd <= 10000):p.error('run requires an explicit positive --budget-usd (maximum 10000)')
 if args.budget_usd is not None:BUDGET_USD=args.budget_usd
